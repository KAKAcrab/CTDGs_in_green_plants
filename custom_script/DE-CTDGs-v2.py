#!/usr/bin/env python3
"""
DE-CTDG-v2: 增强版差异表达串联重复基因簇(DE-CTDGs)分析工具
作者: yyh
日期: 2025-04-12

基于原始DE_CTDG_for_pear.py脚本，增加以下新功能：
1. 与背景基因集表达量的比较（可选功能）
   - 将CTDGs按成员数量分为四组（2, 3-5, 6-10, >10）
   - 对每组随机抽取相同数量的连续基因作为背景CTDGs，每组抽样1000次
   - 判断候选DE-CTDGs的表达值是否高于背景分布的95%分位数或低于5%分位数
   - 使用Wilcoxon秩和检验验证差异的显著性

2. 分析成员数量为2的DE-CTDGs中两个成员的表达差异
...
3.（可选）该CTDG的平均表达量显著高于或低于随机背景基因集的表达分布
...
用法:
    python enhanced_dectdg_analyzer.py \
      ...
      --threads 8 \
      --compare_background \  # 可选，启用背景基因集比较
      --genome_file /path/to/genome.fa  # 可选，用于获取基因组信息以进行抽样
"""
...
def parse_gff(gff_file):
    ...
    返回:
        ...
        gene_positions: 基因ID和染色体位置的字典 (用于背景抽样)
    """
    logger.info(f"解析GFF文件: {gff_file}")
    gene_lengths = {}
    gene_positions = {}  # 存储基因的染色体位置 (chr, start, end)
                ...
                # if parts[2] == "mRNA":
                    ...
                    if gene_id:
                        ...
                        try:
                            ...
                            # 存储基因的染色体位置
                            chromosome = parts[0]
                            gene_positions[gene_id] = (chromosome, start, end)
                        except ValueError:
    ...
    return gene_lengths, gene_positions

def get_gene_order_on_chromosome(gene_positions):
    """
    根据基因在染色体上的位置确定其顺序
    
    参数:
        gene_positions: 基因ID和染色体位置的字典
        
    返回:
        ordered_genes: 按染色体和位置排序的基因列表的字典
    """
    logger.info("根据染色体位置排序基因")
    
    # 按染色体分组基因
    chr_genes = defaultdict(list)
    for gene_id, (chr_id, start, end) in gene_positions.items():
        chr_genes[chr_id].append((gene_id, start, end))
    
    # 对每个染色体上的基因按位置排序
    ordered_genes = {}
    for chr_id, genes in chr_genes.items():
        # 按起始位置排序
        sorted_genes = sorted(genes, key=lambda x: x[1])
        # 只保留基因ID
        ordered_genes[chr_id] = [gene[0] for gene in sorted_genes]
    
    logger.info(f"完成 {len(ordered_genes)} 条染色体上的基因排序")
...
def sample_background_clusters(gene_clusters, cluster_numbers, ordered_genes, size_group, n_samples=1000):
    """
    随机抽样构建背景基因簇分布
    ...
    参数:
        gene_clusters: 基因到簇的映射DataFrame
        cluster_numbers: 簇大小信息DataFrame
        ordered_genes: 按染色体排序的基因列表
        size_group: 基因簇大小分组 (2, 3-5, 6-10, >10)
        n_samples: 抽样次数
        
    返回:
        background_clusters: 背景基因簇列表
    """
    logger.info(f"为大小组 {size_group} 抽样背景基因簇")
    ...
    elif size_group == "member>10":
        min_size, max_size = 11, 20  # 假设最大簇大小为20，根据实际情况调整
    else:
        logger.error(f"无效的簇大小分组: {size_group}")
        return []
    ...
    for _ in range(n_samples):
        if min_size == max_size:
            cluster_sizes.append(min_size)
        else:
            cluster_sizes.append(random.randint(min_size, max_size))
    
    # 为每个目标簇大小随机抽样背景基因
    background_clusters = []
    
    # 获取所有染色体的列表
    chromosomes = list(ordered_genes.keys())
    
    for size in cluster_sizes:
        # 随机选择一条染色体
        chosen_chr = random.choice(chromosomes)
        chr_genes = ordered_genes[chosen_chr]
        
        # 确保有足够的基因可供选择
        if len(chr_genes) < size:
            logger.warning(f"染色体 {chosen_chr} 上的基因数量 ({len(chr_genes)}) 少于所需的簇大小 ({size})")
            continue
        
        # 随机选择起始位置
        start_idx = random.randint(0, len(chr_genes) - size)
        
        # 提取连续的一段基因作为背景簇
        bg_cluster_genes = chr_genes[start_idx:start_idx + size]
        
        # 生成一个唯一的簇ID
        bg_cluster_id = f"bg_cluster_{len(background_clusters)}"
        
        # 将背景簇添加到结果列表
        background_clusters.append({
            'cluster_id': bg_cluster_id,
            'genes': bg_cluster_genes,
            'size': size
        })
    
    logger.info(f"为大小组 {size_group} 抽样了 {len(background_clusters)} 个背景基因簇")
    return background_clusters

def compare_with_background(deg_clusters, cluster_numbers, background_clusters, condition_results, gene_clusters):
    """
    将候选DE-CTDGs与背景基因集进行比较
    
    参数:
        deg_clusters: 候选DE-CTDGs列表
        cluster_numbers: 簇大小信息DataFrame
        background_clusters: 背景基因簇信息
        condition_results: 条件处理结果列表
        gene_clusters: 基因到簇的映射DataFrame
        
    返回:
        significant_clusters: 显著差异的DE-CTDGs字典
    """
    logger.info("比较候选DE-CTDGs与背景基因集")
    ...
    # 对每个条件的DE-CTDGs进行分组
    condition_deg_groups = {}
    
    for result in condition_results:
        condition_name = result['condition_name']
        condition_deg_groups[condition_name] = {
            "member=2": [],
            "3<member<5": [],
            "6<member<10": [],
            "member>10": []
        ...
        # 对该条件下的每个DE-CTDG，根据成员数确定分组
        for cluster in result['deg_clusters']:
            # 获取簇大小
            cluster_size_rows = cluster_numbers[cluster_numbers['cluster_id'] == cluster]
            if not cluster_size_rows.empty:
                try:
                    ...
                    # 将该簇添加到对应分组
                    condition_deg_groups[condition_name][group].append(cluster)
                    
                except (ValueError, TypeError):
                    logger.warning(f"跳过无效簇大小的基因簇: {cluster}")
    
    # 对每个条件，比较DE-CTDGs与背景基因集
    significant_clusters = {}
    
    for result in condition_results:
        condition_name = result['condition_name']
        significant_clusters[condition_name] = {
            "high_expression": [],  # 高于95%分位数
            "low_expression": [],   # 低于5%分位数
            "non_significant": []   # 不显著
        }
        ...
        # 对每个分组进行比较
        for group, clusters in condition_deg_groups[condition_name].items():
            if not clusters:  # 如果该分组没有DE-CTDGs，跳过
                continue
                
            logger.info(f"比较条件 {condition_name} 下大小组 {group} 中的 {len(clusters)} 个DE-CTDGs与背景")
            
            # 获取该分组的背景基因簇
            bg_clusters = [bg for bg in background_clusters if bg['size'] in 
                          ([2] if group == "member=2" else 
                           list(range(3, 6)) if group == "3<member<5" else 
                           list(range(6, 11)) if group == "6<member<10" else 
                           list(range(11, 21)))]
            
            if not bg_clusters:
                logger.warning(f"条件 {condition_name} 下大小组 {group} 没有背景基因簇")
                continue
            ...
            for bg in bg_clusters:
                # 计算该背景簇中所有基因的平均TPM
                bg_genes = bg['genes']
                valid_genes = [gene for gene in bg_genes if gene in treated_tpm.index]
                
                if valid_genes:
                    avg_tpm = treated_tpm[valid_genes].mean()
                    bg_expressions.append(avg_tpm)
            
            if not bg_expressions:
                logger.warning(f"条件 {condition_name} 下大小组 {group} 的背景基因簇没有有效的表达值")
                continue
            
            # 计算背景分布的5%和95%分位数
            q05 = np.percentile(bg_expressions, 5)
            q95 = np.percentile(bg_expressions, 95)
            
            # 比较每个候选DE-CTDG与背景分布
            for cluster in clusters:
                if cluster not in treated_tpm:
                    logger.warning(f"基因簇 {cluster} 在TPM数据中不存在，跳过")
                    continue
                
                # 获取该簇的表达值
                cluster_tpm = treated_tpm[cluster]
                
                # 获取该簇的所有成员基因
                cluster_genes = gene_clusters[gene_clusters['cluster_id'] == cluster]['gene_id'].tolist()
                valid_genes = [gene for gene in cluster_genes if gene in treated_tpm.index]
                
                if not valid_genes:
                    logger.warning(f"基因簇 {cluster} 的成员在TPM数据中不存在，跳过")
                    continue
                
                # 构建该簇的表达值与随机背景的表现值用于Wilcoxon检验
                cluster_expressions = [treated_tpm[gene] for gene in valid_genes]
                
                # 进行Wilcoxon秩和检验
                try:
                    stat, pval = stats.mannwhitneyu(cluster_expressions, bg_expressions)
                    is_significant = pval < 0.05
                except ValueError:
                    logger.warning(f"基因簇 {cluster} 的Wilcoxon检验失败，可能是数据不足")
                    is_significant = False
                
                # 判断该簇的表达是否显著偏高或偏低
                if cluster_tpm > q95 and is_significant:
                    significant_clusters[condition_name]["high_expression"].append(cluster)
                elif cluster_tpm < q05 and is_significant:
                    significant_clusters[condition_name]["low_expression"].append(cluster)
                else:
                    significant_clusters[condition_name]["non_significant"].append(cluster)
...
def create_expression_matrices(all_clusters, condition_results, cluster_numbers, significant_clusters=None):
    """
    创建表达矩阵
    ...
    参数:
        ...
        cluster_numbers: 簇大小信息DataFrame
        significant_clusters: 显著差异的DE-CTDGs字典 (可选)
    ...
    try:
        ...
        for result in condition_results:
            ...
            # 如果有显著差异的DE-CTDGs，只使用显著差异的基因簇
            if significant_clusters and condition_name in significant_clusters:
                sig_high = significant_clusters[condition_name]["high_expression"]
                sig_low = significant_clusters[condition_name]["low_expression"]
                deg_clusters = sig_high + sig_low
            
            if deg_clusters:  # 确保列表不为空
                binary_matrix.loc[binary_matrix.index.isin(deg_clusters), condition_name] = 1
        
        # 添加表达调控方向列 (如果有显著差异的DE-CTDGs)
        if significant_clusters:
            for condition_name, sig_clusters in significant_clusters.items():
                col_name = f"{condition_name}_direction"
                binary_matrix[col_name] = "non-DE"
                
                # 设置高表达簇
                high_expr = sig_clusters["high_expression"]
                if high_expr:
                    binary_matrix.loc[binary_matrix.index.isin(high_expr), col_name] = "up"
                
                # 设置低表达簇
                low_expr = sig_clusters["low_expression"]
                if low_expr:
                    binary_matrix.loc[binary_matrix.index.isin(low_expr), col_name] = "down"
        
        # 重新排列列，使得'size'列在最前面
        cols = binary_matrix.columns.tolist()
        size_cols = ['size']
        direction_cols = [col for col in cols if col.endswith('_direction')]
        data_cols = [col for col in cols if col != 'size' and not col.endswith('_direction')]
        binary_matrix = binary_matrix[size_cols + data_cols + direction_cols]
...
def main():
    ...
    # 新增功能选项
    parser.add_argument("--compare_background", action="store_true", help="启用与背景基因集比较功能")
    parser.add_argument("--n_background_samples", type=int, default=1000, help="背景抽样次数")
    ...
    try:
        # 1. 加载基因长度信息和位置信息 (用于背景抽样)
        gene_lengths, gene_positions = parse_gff(args.gff_file)
        ...
        # 5. 如果启用背景比较，按染色体位置排序基因
        ordered_genes = None
        if args.compare_background:
            logger.info("准备基因组背景数据用于比较")
            ordered_genes = get_gene_order_on_chromosome(gene_positions)
        ...
        # 7. 如果启用背景比较，执行背景基因集比较分析
        significant_clusters = None
        if args.compare_background and ordered_genes:
            logger.info("开始与背景基因集比较分析")
            
            # 为每个大小组构建背景基因集
            background_clusters = []
            for size_group in ["member=2", "3<member<5", "6<member<10", "member>10"]:
                bg_clusters = sample_background_clusters(
                    gene_clusters, 
                    cluster_numbers, 
                    ordered_genes, 
                    size_group, 
                    n_samples=args.n_background_samples
                )
                background_clusters.extend(bg_clusters)
            
            # 将候选DE-CTDGs与背景比较
            significant_clusters = compare_with_background(
                all_clusters, 
                cluster_numbers, 
                background_clusters, 
                condition_results,
            ...
            for condition_name, sig_clusters in significant_clusters.items():
                # 高表达簇
                high_expr_file = os.path.join(args.output_dir, f"{condition_name}_high_expression_clusters.txt")
                with open(high_expr_file, 'w') as f:
                    for cluster in sig_clusters["high_expression"]:
                        f.write(f"{cluster}\n")
                logger.info(f"{condition_name} 条件下高表达的DE-CTDGs列表已保存到: {high_expr_file}")
                
                # 低表达簇
                low_expr_file = os.path.join(args.output_dir, f"{condition_name}_low_expression_clusters.txt")
                with open(low_expr_file, 'w') as f:
                    for cluster in sig_clusters["low_expression"]:
                        f.write(f"{cluster}\n")
                logger.info(f"{condition_name} 条件下低表达的DE-CTDGs列表已保存到: {low_expr_file}")
        
        # 8. 分析成员数量为2的DE-CTDGs中两个成员的表达差异
